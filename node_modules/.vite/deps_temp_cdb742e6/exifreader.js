...
function read10(dataView, tiffHeaderOffset, offset, includeUnknown) {
...
var EXIF_OFFSET = 6;
function read12(dataView, pngTextChunks, async, includeUnknown) {
  const tags = {};
  const tagsPromises = [];
  ...
  if (constants_default.USE_EXIF && hasExifData(tiffHeaderOffset)) {
    ...
    if (constants_default.USE_MAKER_NOTES && readTags2["MakerNote"]) {
      if (hasCanonData(readTags2)) {
        ...
        const readPentaxTags = pentax_tags_default.read(dataView, tiffHeaderOffset, readTags2["MakerNote"].__offset, includeUnknown);
        if (expanded) {
